...
def ensure_filename_present(document_item):
    """
    Asegura que el atributo 'filename' esté presente en el diccionario del documento,
    decodificándolo desde 'encoded_filename' o extrayéndolo de 's3_key' si es necesario.
    Modifica el diccionario del documento directamente.

    Args:
        document_item (dict): El diccionario del item del documento.
    
    Returns:
        dict: El mismo diccionario del item del documento, posiblemente modificado.
    """
    if not isinstance(document_item, dict):
        logger.warning("ensure_filename_present recibió un item que no es un diccionario.")
        return document_item # Devolver sin cambios si no es un dict

    # Prioridad 1: si ya tiene 'filename', no hacer nada (asumimos que es correcto)
    if 'filename' in document_item and document_item['filename']:
        return document_item

    # Prioridad 2: si tiene 'encoded_filename', usarlo para obtener 'filename'
    if 'encoded_filename' in document_item and document_item['encoded_filename']:
        document_item['filename'] = decode_s3_key(document_item['encoded_filename'])
        return document_item
    
    # Prioridad 3: si tiene 's3_key', extraer el nombre de archivo de ahí
    if 's3_key' in document_item and document_item['s3_key']:
        document_item['filename'] = extract_filename_from_key(document_item['s3_key'])
        return document_item
    
    # Si no se pudo determinar, loguear y devolver el item como estaba
    logger.warning(f"No se pudo determinar 'filename' para el documento ID: {document_item.get('id', 'Desconocido')}")
    return document_item