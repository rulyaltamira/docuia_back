...
def lambda_handler(event, context):
    ...
    path = event.get('path', '')
    logger.info(f"STATS_MANAGER: método {http_method}, ruta {path}")

    if http_method == 'OPTIONS':
        return {'statusCode': 200, 'headers': add_cors_headers({}), 'body': ''}
    
    if http_method == 'GET' and path == '/stats/summary':
        return get_summary_stats_api_version(event, context)
    elif http_method == 'GET' and path == '/stats/documents':
        return get_document_stats_api_version(event, context)
    elif http_method == 'GET' and path == '/stats/users':
        return get_user_stats_api_version(event, context)
    elif http_method == 'GET' and path == '/stats/alerts':
        return get_alert_stats_manager_version(event, context) 
    elif http_method == 'GET' and path == '/stats/processing':
        return get_processing_stats_api_version(event, context)
    elif http_method == 'GET' and path == '/stats/storage':
        return get_storage_stats_api_version(event, context)
    elif http_method == 'GET' and path == '/stats/critical-documents':
        return get_critical_documents_api_version(event, context)
    elif http_method == 'GET' and path == '/stats/trends':
        return get_trends_stats_api_version(event, context)
    elif http_method == 'GET' and path == '/stats/key-dates':
        return get_key_dates_api_version(event, context)
    elif http_method == 'GET' and path == '/stats/risks': 
        return get_risk_stats_api_version(event, context)
    else:
        logger.warning(f"Operación no válida STATS_MANAGER: {http_method} {path}")
        return {'statusCode': 404, 'headers': add_cors_headers({'Content-Type': 'application/json'}), 'body': json.dumps({'error': 'Ruta stats no encontrada'})}
